...
uint256 hashGenesisBlock("0x00000000000000000000000000000000a9823e2965f9d90e88fdc0998adf4ac1");//hgb
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // GlobalDenomination: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "GlobalDenomination Signed Message:\n";
...
{
    // GlobalDenomination: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 700000); // GlobalDenomination: 700,000

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 4 * 60 * 60; // GlobalDenomination: 4 hours
static const int64 nTargetSpacing = 60; // GlobalDenomination: 1 minute
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // GlobalDenomination: This fixes an issue where a 51% attack can change difficulty at will.
...
{
        static const int64 BlocksTargetSpacing = 60; // GlobalDenomination: 1 minute
        static const unsigned int TimeDaySeconds = 24 * 60 * 60; // GlobalDenomination: 1 day
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
        if (fTestNet) {
            ...
            if (pindexLast->nHeight + 1 >= 1) {
                if(pindexLast->nHeight > DIFF_SWITCH_BLOCK) {
                    DiffMode = 3; //DS
                } else {
                    DiffMode = 2;
                }
...
bool CBlock::AcceptBlock(CValidationState &state, CDiskBlockPos *dbp)
{
    ...
    if (hash != hashGenesisBlock) {
        ...
        printf("height: %d\n",nHeight);
        printf("AcceptBlock: n1=%.20lf n2=%.20lf\n", n1, n2);
        printf("nBits=%u nBitsNext=%u\n", nBits, nBitsNext);
...
{
    // GlobalDenomination: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x00000088b52626b8f6556f30d4c26244a8b4ec988be03eeecdc2cf59a18a32c0");
    }
...
bool InitBlockIndex() {
    ...
    // vMerkleTree: b0019d92bc054f7418960c91e252e7d24c77719c7a30128c5f6a827c73095d2a 
		...
		const char* pszTimestamp = "Tritium";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
 ...
 if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x1992822d251264537421d8d282358ae4177c3f26dcfaca390146fbf7ba85d7ba"));
...
unsigned char pchMessageStart[4] = { 0xfe, 0xc3, 0xb9, 0xde }; // GlobalDenomination: increase each by adding 6 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    //// debug print
    printf("GlobalDenominationMiner:\n");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("GlobalDenominationMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GlobalDenominationMiner : ProcessBlock, block not accepted");
    }
